...
- **Toolchain:** the default rootfs ships Rust Stable 1.96.0 `/bin/rustc` as the official stage2 Ristux-hosted compiler built by `make rust-official-rustc`, a Ristux-native local-package `/bin/cargo` frontend, the bootstrap `/bin/rustdoc` frontend, official-bootstrap `rust-core-libs` and `rust-std-libs`, maintained Ristux Rust overlay sources, and the pure Rust `ristux-ld` static ELF linker. Installed VM boot and smoke fixtures verify native no-std and hosted-std compile/link/execute, `cargo new --std` plus `cargo run`, editions 2015/2018/2021/2024, and host-built `std::thread::spawn(...).join()` execution. The local Cargo frontend supports binary/library projects, recursive path dependencies, and explicit workspace members. Upstream Cargo now passes the vendored `getrandom` blocker; the saved bootstrap log directly reaches C-backed zlib and nghttp2, while the remaining transport graph also includes OpenSSL, curl, and Git dependencies. Registry/Git, wildcard workspace members, build scripts, and proc macros remain pending. The old C libc/CRT, TinyCC, Newlib, and Dropbear probes are removed from the default tree.
