...
- **Toolchain:** the default rootfs ships Rust Stable 1.96.0 `/bin/rustc` as the official stage2 Ristux-hosted compiler built by `make rust-official-rustc`, an upstream Cargo 1.96.0 `/bin/cargo` built by `make rust-official-cargo`, the bootstrap `/bin/rustdoc` frontend, official-bootstrap `rust-core-libs` and `rust-std-libs`, maintained Ristux Rust overlay sources, and the pure Rust `ristux-ld` static ELF linker. Cargo is built in a pure-Rust Ristux-offline configuration: its curl/libgit2 C graph is replaced by target compatibility layers, Git repository/fetch/checkout operations use gix, and global SQLite cache tracking is disabled on Ristux while local build/cache locks remain. Installed VM boot and smoke fixtures verify native no-std and hosted-std compile/link/execute, upstream `cargo new` plus `cargo run`, editions 2015/2018/2021/2024, recursive path and local `file://` Git dependencies, explicit workspaces, build scripts, and host-built `std::thread::spawn(...).join()` execution. Network Git transports, registry HTTPS, and proc macros remain pending. The old C libc/CRT, TinyCC, Newlib, and Dropbear probes are removed from the default tree.
