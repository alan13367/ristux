...
pub fn write_bytes(bytes: &[u8]) -> usize {
    ...
    if let FramebufferMode::Linear(framebuffer) = mode {
        for (index, color) in bytes.chunks(3).take(64).enumerate() {
            ...
            write_pixel(framebuffer, index % 16, index / 16, pack(framebuffer.layout, red, green, blue));
        }
    }
...
fn stroke_rect(buffer: &mut BackBuffer, x: usize, y: usize, width: usize, height: usize, color: u32) {
