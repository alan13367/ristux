...
if [[ $# -ne 1 ]]; then
  echo "usage: scripts/build_ristux_ssh.sh <output-elf>" >&2
  exit 2
fi
...
manifest="$PWD/toolchain/ristux-ssh/Cargo.toml"
target_dir="${RISTUX_SSH_TARGET_DIR:-$PWD/build/ristux-ssh-target}"
mkdir -p "$(dirname "$output")" "$target_dir"

# Cargo discovers configuration from its current directory. Build from /tmp so
# the kernel's no-std root configuration cannot override this hosted target.
(
  cd /tmp
  PATH="$host_linker_dir:$(dirname "$host_objcopy"):$PATH" \
    RUSTC="$host_rustc" \
    CARGO_TARGET_DIR="$target_dir" \
    CARGO_TARGET_X86_64_UNKNOWN_RISTUX_RUSTFLAGS='--cfg getrandom_backend="custom"' \
    cargo +1.96.0 build \
      --locked \
...
cp "$target_dir/$target/release/ssh" "$output"
chmod 755 "$output"
echo "pure-Rust Ristux SSH client built at $output"
