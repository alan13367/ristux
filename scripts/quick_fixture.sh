...
BOOT_WAIT="${RISTUX_QUICK_BOOT_WAIT:-20}"
...
case "$SCENARIO" in
  ...
  filesync)
    ...
    EXPECTS=(
      ...
      "cc_file_sync: directory read errno ok"
      "cc_file_sync: directory write open errno ok"
  ...
  futex)
    ...
    EXPECTS=(
      ...
      "cc_futex: unsupported flags ok"
      "cc_futex: pointer validation ok"
      "cc_futex: wake waiter ok"
      "cc_futex: private namespace ok"
      "cc_futex: no wake changed value ok"
      "cc_futex: signal wait ok"
      "cc_futex: nanosleep invalid ok"
      "cc_futex: nanosleep overflow ok"
      "cc_futex: nanosleep yield ok"
      "cc_futex: nanosleep interrupt ok"
      "cc_futex: done"
    )
    ;;
  signal)
  ...
  proc)
    ...
    EXPECTS=(
      ...
      "cc_proc: clone tls ok"
      "cc_proc: preemptive scheduling ok"
      "cc_proc: elf permissions ok"
      "cc_proc: user fault containment ok"
      "cc_proc: exec vector limits ok"
      "cc_proc: exec vector faults ok"
      "cc_proc: exec long strings ok"
  ...
  kernel-prims)
    ...
    EXPECTS=(
      ...
      "^cc_select: timeout writeback ok$"
  ...
  toolchain)
    ...
    EXPECTS=(
      ...
      "^ristux-ld: self-test linked archive/rlib input$"
      "^rust_host_probe: toolchain files ok$"
      "^rust_host_probe: manifest ok$"
      "^rust_host_probe: target spec ok$"
      "^rust_host_probe: package index ok$"
      "^rust_host_probe: environment ok$"
      "^rust_host_probe: fd flags ok$"
      "^rust_host_probe: file io ok$"
      "^rust_host_probe: std syscalls ok$"
      "^rust_host_probe: cargo fs syscalls ok$"
      "^rust_host_probe: process signal syscalls ok$"
      "^rust_host_probe: tls syscalls ok$"
      "^rust_host_probe: host runtime syscalls ok$"
      "^rust_host_probe: host platform syscalls ok$"
      "^rust_host_probe: dir traversal ok$"
      "^rust_host_probe: sysroot libs ok$"
      "^rust_host_probe: overlay sources ok$"
      ...
      "^  ristux-ld$"
      "^  /bin/rustc$"
      "^  /usr/lib/rustlib/rust-1\\.96\\.0-manifest\\.toml$"
      ...
      "^  /bin/rustdoc$"
      "^name: ristux-ld$"
      "^version: 0\\.3\\.0-bootstrap$"
      "^  /bin/ristux-ld$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/bin/ristux-ld$"
      "^name: rust-host-probe$"
      "^  rustc$"
      "^  cargo$"
      "^  rustdoc$"
      "^  /bin/rust_host_probe$"
      "^name: rust-core-libs$"
      "^version: 1\\.96\\.0$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/libcore-.*\\.rlib$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/liballoc-.*\\.rlib$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/libcompiler_builtins-.*\\.rlib$"
      "^name: rust-std-libs$"
      "^version: 1\\.96\\.0$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/libstd-.*\\.rlib$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/libstd-.*\\.rmeta$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/libpanic_abort-.*\\.rlib$"
      "^  /usr/lib/rustlib/x86_64-unknown-ristux/lib/liblibc-.*\\.rlib$"
      "^  rustc$"
      "^  ristux-ld$"
      "^  rust-ristux-overlays$"
      "^name: rust-ristux-overlays$"
      "^version: 1\\.96\\.0$"
      "^  /usr/lib/rustlib/src/ristux-overlays/README\\.md$"
      "^  /usr/lib/rustlib/src/ristux-overlays/libc/src/unix/ristux_syscalls\\.rs$"
      "^  /usr/lib/rustlib/src/ristux-overlays/probe/restricted_main\\.rs$"
      "^  /usr/lib/rustlib/src/ristux-overlays/rust-src/library/std/src/sys/alloc/unix\\.rs$"
  ...
  command)
    ...
    ;;
  *)
    echo "unknown scenario '$SCENARIO' (try boot, toolchain, rust-std, make, pkgconf, loopback, ssh-banner, or command)" >&2
    exit 2
    ;;
esac
